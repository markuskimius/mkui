...
Numeric alignment: all-numeric columns right-align with per-cell right padding (`--mkui-num-pad`, ch) so decimals line up (`colStats.maxFrac`); the filter dropdown pads left alike. Flash classes: `mkui-flash-in`/`-out`/`-update`. Each pane gets a unique `subid`.
...
Disconnected indicator: the table subscribes to `mkio.connected`; a drop in live mode shows "Disconnected" for the Live dot, live mode staying on. Declare it *after* the paging variables: `State.subscribe` fires its initial callback synchronously.
