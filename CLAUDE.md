...
- Tab strip look: flush tabs, rounded top corners, `::after` bottom flares colored by `--mkui-tab-bg`. The bar's bottom line is a `.mkui-tabbar::after` overlay (never a border) that the selected tab covers (`z-index: 1`); selected tabs outside the focused group flatten to idle. (`tests/styles.test.js`)
...
- `lib/icons.js` — `icon(name)` returns a currentColor `<svg>` from vendored path data (Lucide outlines + custom shapes)
...
- `display` — column → template for what a cell *shows*; see Display templates
...
- `filters` — column → default filter (values, a range, or a preset); see Configured filters
- `sort` — a column name (`"-col"` descending), `{ col, dir }`, or an array; see Configured sort
- `visible` — the columns to show, in order; absent shows all; see Column visibility
- `groups` — picker categories, `[{ label, columns }]`; see Column groups
...
Row identity: query `_mkio_row`, stream `_mkio_ref`, subpub `_mkio_topic`; `_mkio_*` columns hidden. The header renders at init with `columns`, else on the first row.
...
Sorting: click a header to cycle ascending → descending → none, shift+click adding secondary keys, the priority a digit in the caret (`.mkui-sort-num`). Numeric vs string comparison is auto-detected; new rows insert sorted.
...
Virtualized rows: only rows overlapping the viewport (plus overscan) exist in the DOM, two `.mkui-vspacer` rows carrying the rest's height. Data: a `rows` Map plus `baseOrder`; `view` is the filtered+sorted key array driving rendering. `render()` reconciles the visible slice, reusing keyed `tr`s. Inserts/deletes/replaces patch `view`, sort/filter changes dirty it.
...
Paging (stream): with `maxcount` (default 200) a toolbar shows `◀ Earlier | time range | Later ▶ | ● Live | ⟳`. The first fetch starts at local midnight (`start: "today"`, a UTC ref) or the buffer's start (`""`). The range label shows visible rows' local timestamps at adaptive precision (cross-day adds the date; `No data` when empty) plus `(start)`/`(end)`/`(all)`.
...
`● Live` resumes streaming on the main `subid` from the page's `lastRef`, and disables Later; Earlier fetches through a separate `pageSubId` (`fetchPrevLive`), prepending rows without a stream break (`pageFetchPending` guards). Exiting unsubscribes both and re-fetches the saved page; sort, filter, column order persist.
...
Visibility-aware subscriptions: an `IntersectionObserver` gates the subscription — a hidden pane subscribes when shown; hidden 5 minutes drops it. `mkui-pane-close` sets `closed`, disconnects the observer, unsubscribes; `mkui-pane-open` clears `closed`, drops stale rows/sort/filter/paging/tree/find state (links stay), re-observes.
...
Snapshot clearing: query and subpub `applySnapshot` clears rows, DOM, selection first — auto-reconnect fires `onSnapshot` without `sub()`, so outage deletes would linger; streams append.
