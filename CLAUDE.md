...
Numeric alignment: all-numeric columns right-align, per-cell padding (`--mkui-num-pad`, ch) lining the decimals up (`colStats.maxFrac`), dropdown alike. Flashes `mkui-flash-in`/`-out`/`-update`/`-undo`; each pane gets a unique `subid`.
...
Clipboard: `copySelection` builds a grid — row mode: selected rows × visible columns plus a `labels` header; cell mode: the bounding grid, blanks outside the rects; else the focused cell — as TSV and HTML (`lib/copy.js`; over 100k rows skip HTML).
...
Configured filters: `filters = { col = <filter> }` seeds them at init (before data) and on `mkui-pane-open`. `filterFromSpec(col, spec)` reads the README's shapes — values from a list / `{ include }` / `{ exclude }`, a range from `from`/`to`/`empty`/`preset` framed by `types[col].type` else the entry's `type` else the bounds, through `inputToBound`; `off = true` ships one switched off. A bad entry warns `bad filters.<col>`; `null`/`""` clears. `filterToSpec` is the inverse: `getFilters()` round-trips through `setFilters(map, { merge })` (replace by default; `merge` keeps other columns, `null` clears one). Hook `_filters`; `workspace.setPaneFilters(id, filters, opts)` (builds a never-shown pane first; `id == null` → the focused pane), `getPaneFilters(id)`; `table.filter`.

Embedding: `_source = { set({ filter, topic }), get }` re-aims a table at another slice of its service (unsub, clear, re-subscribe: the server answers for the new slice); `_data = { rows, view, selected, on }` reads what it holds, `on` coalescing to one call per task; `_toolbar = { extras, sync }` takes an embedder's controls into the toolbar (made on demand; `sync` — an empty toolbar is not in the DOM).
...
Paging (stream): with `maxcount` (200) a toolbar shows `◀ Earlier | time range | Later ▶ | ● Live | ⟳`; the first fetch starts at local midnight (`start: "today"`, a UTC ref) or the buffer's start (`""`). The range label gives visible rows' local times at adaptive precision plus `(start)`/`(end)`/`(all)`.
...
Visibility-aware subscriptions: an `IntersectionObserver` gates it — a hidden pane subscribes when shown, 5 minutes hidden drops it. `mkui-pane-close` sets `closed`, disconnects it and unsubscribes; `mkui-pane-open` clears `closed`, drops stale rows/sort/filter/paging/tree/find/undo state (links stay), re-observes.
