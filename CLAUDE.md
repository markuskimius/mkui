...
- Icons are inline SVGs from `lib/icons.js` (`icon(name)`), not text glyphs: `currentColor`, sized by `.mkui-icon` CSS, whose `pointer-events: none` lands hits on the hosting button (`tests/styles.test.js`)
