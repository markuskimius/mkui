...
- Tab overflow: past `min-width: 3em` `.mkui-tabs` clips, `.mkui-tab-scroll` arrows appear and the bar gets `.mkui-tabbar-overflow` (`updateArrows`). Rename: ctrl/cmd+click swaps the label for `.mkui-tab-rename`; `workspace.renamePane` sets `titled`, which stops `setPaneAutoTitle` overwriting a chosen name.
