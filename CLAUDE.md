...
- Tab overflow: tabs shrink to `min-width: 3em`; when the strip still overflows, `.mkui-tabs` clips and scroll arrows (`.mkui-tab-scroll`) appear, the bar getting `.mkui-tabbar-overflow` (`updateArrows` in `_renderTabBar`, which also scrolls the active tab into view). At least one tab always shows.
...
When `config.auth` is present, `<mkui-app>` shows a login dialog before loading frames. `method: "mkio"` (default) calls `client.auth({username, password})` against mkio's `_mkio_users` (seed users `admin`/`password`, `user`/`password`); `method: "custom"` uses `app.registerAuthHandler({ authenticate({username, password}) })` returning `{ user, role }`.
...
Login dialog: a floating frame (`stayOnTop`, `noDock`) with `_hideClose = true` and `_extraControls = () => []` so it can't be dismissed.
...
Clipboard: `copySelection` builds a grid — row mode: selected rows × visible columns plus a `labels` header row; cell mode: bounding grid with blanks outside the rects; fallback: the focused cell — and writes `text/plain` TSV and `text/html` via `ClipboardItem` (`lib/copy.js`; `writeText` fallback; grids over 100k rows skip HTML). `status.message` shows "Copied N rows/cells".

Buttons: `enable.when = "<expr>"` gates a button on the selection — scope `rows`, `row` (first or NULL), `cells`, `selection` `{ count, rowCount, cellCount, unit }`, `connected`, `state` — with the structural flags `connected`, `minSelected`, `maxSelected`. `unit`: `"rows"` (default), `"row"`, `"cells"`, `"cell"` — singular units default min/max to 1. Row units receive the rows the selection implies (explicit rows, else rows containing selected cells, else the focused cell's row). Cell units get `cells: [{ row, column, value }]` plus `cell` in the action context; cell-unit transactions send one request per cell. Gates re-evaluate on selection, filter, and connection changes and on a live replace/delete of a row in the selection (`rowInSelection`).

Sorting: click a header to cycle ascending → descending → none; shift+click adds secondary keys. Multi-sort priority is a digit knocked out *inside* the caret (`.mkui-sort-num`). Numeric vs string comparison is auto-detected. New rows insert at the sorted position.
...
`live: true` still fetches the start page first and hands off from its `onPage` (`autoLivePending` — consumed on the first page, re-armed on pane reopen); going live straight from `sub()` would replay the whole buffer. An empty start page leaves `lastRef` null, so the handoff seeds it from `getStartRef()`.

Disconnected indicator: the table subscribes to `mkio.connected`; when the socket drops in live mode the toolbar shows "Disconnected" instead of the Live dot, and live mode stays on. The subscription must be declared *after* all paging variables — `State.subscribe` fires its initial callback synchronously.
