...
Numeric alignment: all-numeric columns right-align, per-cell padding (`--mkui-num-pad`, ch) lining the decimals up (`colStats.maxFrac`), dropdown too. Flashes `mkui-flash-in`/`-out`/`-update`/`-undo`; each pane gets a unique `subid`.
...
Filtering: each column header has one icon slot — the filter button, pinned right, a hamburger until the column is sorted, when `updateHeaderState` swaps in the sort caret. Either opens the same panel, placed by `placeDropdown`. Lists open at content height, capped by `fitList` above `dropdownFloor()` via an inline `max-height` that `resize: vertical` honours; a dragged height is kept per kind. Changes apply at once, filters persist across resubscribes. A values filter records intent (`mode: "include" | "exclude"`): the dropdown starts (and "Select all" resets) in exclude mode, where unchecking hides those values and unseen ones pass; "Clear" flips to include mode, where checking shows only those. An empty exclusion is no filter, an inclusion always is.

Off filters: `f.off` suspends one without unmaking it — kept, chipped (`.mkui-chip-off`; `describeFilter(f, { state })` puts `(off)` in the tooltip, not the chip label), applied by nothing, so every read skips it (`matchesFilters`, `allScopeActive`, `syncPresetTimer`, `flatRanked` — all via `anyFilterOn`). `setFiltersOn(keys, on)` from the chip check, the colops `.mkui-filter-applied` box, `toggleAllFilters`. Header btn `.mkui-filter-off` when a column's are all off (CSS after `.active`/`.mkui-filter-linked`, so off wins). `×`/`clearFilters` still drop.
...
Virtualized rows: only those overlapping the viewport (plus overscan) exist in the DOM, two `.mkui-vspacer` rows carrying the rest's height. Data: a `rows` Map plus `baseOrder`; `view` is the filtered+sorted key array driving rendering, `render()` reconciling the shown slice with keyed `tr`s. Inserts/deletes/replaces patch it, sort/filter changes dirty it.
