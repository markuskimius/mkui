...
Numeric alignment: all-numeric columns right-align with per-cell right padding (`--mkui-num-pad`, ch) so decimals line up (`colStats.maxFrac`), dropdown alike. Flashes: `mkui-flash-in`/`-out`/`-update`. Each pane gets a unique `subid`.
...
Column reorder: drag a header (5px threshold), ghost + drop indicator; the order lands in `visible`.
