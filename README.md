...
The built-in types declare their keys; a custom type declares its own as
`registerPaneType`'s third argument, and one that declares none is not
checked. A pane of `widgets` or `content` takes `title` alone.
...
- **What is sent.** The rows the selection implies — selected rows, else
  ...
  rows send several values; nothing selected retracts the name, and so does a second Escape in the
  table, withdrawing the cursor's row until the next click or arrow key. In a
...
- **Controls.** The toolbar shows one chip per direction — *Broadcast:
  ...
  when it holds several names); the group's icon removes every link. Ctrl/cmd-click a
...
- Custom pane types are the primary extensibility surface. Register with
  `registerPaneType(name, factory, keys)`; reference from config as `type = "<name>"`.
  `keys` lists the config keys the type reads, so a pane carrying any other
  key is reported (below); a type that declares none is not checked.
