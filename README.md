...
- **Ctrl/Cmd+C, Ctrl/Cmd+A, Ctrl/Cmd+F, Escape** → copy, select-all,
  find, and clear-selection, routed to the focused frame's active pane
  (tables implement all four; Ctrl/Cmd+F is taken over from the
  browser, whose own find can't see a virtualized table's off-screen
  rows). Either modifier works on every platform; text inputs and
